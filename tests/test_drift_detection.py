...
def test_load_dataset_from_csv_ok(tmp_path: Path):
    ...
    y_train = pd.DataFrame({"target": [0.0, 1.0, 0.0, 1.0]})
    ...
    # y_train_for_drift debe existir
    assert hasattr(det, "y_train_for_drift")

    # Misma cantidad de filas en todos los datasets
    n = len(X_train)
    assert det.X_train.shape[0] == n
    assert det.X_drift.shape[0] == n
    assert len(det.y_train_for_drift) == n

    # Columnas: X_drift debe estar alineado con X_train (sin "extra")
    assert list(det.X_train.columns) == ["num", "cat"]
...
def test_load_dataset_uses_dataframe_source(tmp_path: Path):
    ...
    X_train = pd.DataFrame({"x": [1, 2, 3], "y": [0.5, 0.7, 0.9]})
    y_train = pd.DataFrame({"target": [0.0, 1.0, 0.0]})
...
def test_load_dataset_raises_if_missing_columns_in_drift(tmp_path: Path):
    """Debe lanzar ValueError si X_drift no contiene todas las columnas de X_train."""
...
def test_drift_report_contains_expected_structure_and_flags(tmp_path: Path):
    ...
    y_train = pd.DataFrame({"target": np.random.randint(0, 2, size=n)})
